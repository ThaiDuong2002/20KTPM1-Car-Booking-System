...
const NotificationController = {
    async add_notification(req, res, next) {
        try {
            ...
            // Create notification
            const notification_re = await NotificationService.create_notification(notificationInfo);
            res.status(201).json({
    ...
    async update_notification(req, res, next) {
        try {
            ...
            // Update notification
            const update_result = await NotificationService.update_notification(notification_id, updateInfo);
            if (!update_result) {
