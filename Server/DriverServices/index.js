...
io.on('connection', (socket) => {
  ...
  socket.on('driverLocationUpdate', (location) => {
    console.log('Received updated location from driver: ', location);
    // Emit the new location to all connected clients
    io.to("sockettID").emit('updateDriverLocation', location);
    
  });
  socket.on('chatdriver', (msg) => {
    // client 1 gửi lên server
    // "Tài xế tới chưa"
    // 
  });
 // isolated 
